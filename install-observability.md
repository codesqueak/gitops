...
| JVM (Micrometer) | GC "max pause" and HTTP request "Duration - MAX" lines (both use `histogram_quantile(1.0, ...)`) | Checked 2026-07-13 while fixing the HTTP metric naming (above): both `jvm_gc_pause_milliseconds_bucket` and `http_server_requests_milliseconds_bucket` export exactly one bucket (`le="+Inf"`) - i.e. a plain count, not a real histogram with distinguishable boundaries. `histogram_quantile()` can't interpolate a quantile from a single bucket and returns `NaN` for every point, which renders as a gap. This isn't a query bug - it needs the app's Micrometer/OTel config to actually emit percentile-histogram bucket boundaries (e.g. `management.metrics.distribution.percentiles-histogram.*` or an equivalent OTel histogram aggregation view); not fixed here since it's an app-side change outside this repo |
