...
- This repo cloned and pushed to GitHub (ArgoCD pulls from `https://github.com/codesqueak/gitops.git`)
...
| Kubernetes / Views / Global | 15757 | Cluster/namespace/pod resource view built on `kube-state-metrics`, actively maintained (dotdc/grafana-dashboards-kubernetes), designed for kube-prometheus-stack but works against Mimir since it's Prometheus-compatible. Required both infra-side and dashboard-side fixes — see [Dashboard fixes](#dashboard-fixes) |
...
| Logs / App | 13639 | Generic Loki log viewer (filter by namespace/pod/container) — covers ad-hoc log browsing without needing to hand-write LogQL in Explore every time. Required a dashboard-side fix — see [Dashboard fixes](#dashboard-fixes) |
...
|---|---|---|---|
| Node Exporter Full | Blank — Alloy's node-exporter scrape target used the wrong Kubernetes Service DNS name (`prometheus-node-exporter...` instead of the chart's actual `node-exporter-prometheus-node-exporter...` fullname) | Corrected the scrape target address in `gitops-repo/observability/alloy/values/dev.yaml`. Later replaced entirely with label-based endpoint discovery — see [Scrape target portability](#scrape-target-portability) | No |
| Kubernetes / Views / Global | Two independent problems. **Infra-side**: per-pod/per-container CPU, memory and network panels (roughly half the dashboard) blank — Alloy only scraped `kube-state-metrics` and `node-exporter`, never the kubelet's cAdvisor endpoint that `container_*`/`machine_*` metrics come from. **Dashboard-side** (found 2026-07-13): the Cluster/Global CPU and Memory Utilization gauges, the "by instance" panels, and the network panels were blank even after the cAdvisor fix — all filter on `job="$job"`, but the dashboard's `$job` variable is declared `multi: true`, and Grafana always regex-escapes `multi: true` variables on interpolation (even with a single value selected), on the assumption they're used with `=~`. Alloy's default job label for a `prometheus.scrape` block is the dotted component path (e.g. `prometheus.scrape.node_exporter`), so the escaper turned those dots into `\.`, which then never equals the real (unescaped) label value under `=`. Invisible against stock kube-prometheus-stack, whose node-exporter job is plainly `node-exporter` (no regex metacharacters to escape) | **Infra**: added `discovery.kubernetes` (role=node) + `discovery.relabel` + `prometheus.scrape "cadvisor"` to Alloy, scraping each node's kubelet directly at `:10250/metrics/cadvisor` using the pod's own service-account bearer token (the chart's default ClusterRole already grants the `nodes/metrics` permission this requires). **Dashboard**: forked into `gitops-repo/observability/grafana/dashboards/kubernetes-views-global.json` with every `job="$job"` changed to `job=~"$job"` (25 occurrences) | Yes |
| JVM (Micrometer) | Blank/partially blank for two independent reasons. **App-side**: Micrometer's OTLP metrics registry defaulted to pushing to `localhost:4318` (nothing listens there — Alloy is a separate pod), and even once pointed at Alloy, its Service didn't expose the OTLP ports (4317/4318) it listens on internally, only its debug port (12345) — so traces were silently broken too, not just this. **Dashboard-side**: this dashboard assumes the classic `micrometer-registry-prometheus` naming convention, which doesn't match the OTel-derived names/units this stack's `spring-boot-starter-opentelemetry` → Alloy → Mimir path actually produces — e.g. `jvm_threads_live_threads` vs. the real `jvm_threads_live`, `jvm_gc_pause_seconds_*` vs. the real `jvm_gc_pause_milliseconds_*`. Metrics also had no `application` label at all (Spring Boot doesn't add it automatically), so the dashboard's app picker resolved to empty | **App**: added `management.otlp.metrics.export.url` and `management.metrics.tags.application` to the service's `application.properties`. **Infra**: added `alloy.extraPorts` (4317, 4318) in `gitops-repo/observability/alloy/values/dev.yaml` so Alloy's Service exposes what it already listens on. **Dashboard**: forked into `gitops-repo/observability/grafana/dashboards/jvm-micrometer.json` with every affected query renamed/rescaled to the real metric names; the GC "max pause" panel has no direct equivalent under OTel's histogram-based export and is approximated with `histogram_quantile(1.0, ...)`. **Later (2026-07-13)**: found two more issues on this same forked dashboard. (1) 14 panels (I/O Overview, GC, `logback_events_total`, `jvm_classes_loaded`) hardcoded a literal `[1m]` window for `rate()`/`increase()`/`delta()`/`resets()` - the same class of bug as the Mimir datasource's `$__rate_interval` fix below, but hardcoded directly in the query text so that fix doesn't reach it. Widened every one to `[4m]`. (2) The I/O Overview panels still used the classic `http_server_requests_seconds_*` names; the real OTel-derived metrics are `http_server_requests_milliseconds_*` (same seconds-vs-milliseconds mismatch as the GC metrics). Renamed "Rate"/"Errors"/"Duration" to match, added the `/ 1000` rescale to "Duration"'s average, and rebuilt its "max" line with the same `histogram_quantile(1.0, ...)` approximation used for GC max pause (`http_server_requests_seconds_max` has no OTel equivalent, same as `jvm_gc_pause_seconds_max`). **Later still (2026-07-13)**: those two `histogram_quantile(1.0, ...)` lines were returning `NaN` — `jvm_gc_pause_milliseconds_bucket` and `http_server_requests_milliseconds_bucket` only ever exported a single `le="+Inf"` bucket (a plain count, no real boundaries), because Micrometer's percentile-histogram publishing defaults to off per-meter. Initially misdiagnosed as an app-side gap outside this repo's reach; turned out to be fixable via env var alone. First attempt (`MANAGEMENT_METRICS_DISTRIBUTION_PERCENTILES_HISTOGRAM_HTTP_SERVER_REQUESTS=true` etc.) crashed the app on boot — Spring Boot's env-var relaxed binding is ambiguous between the `percentiles-histogram` map key and the separate `percentiles` map (`Map<String, double[]>`), and it bound `"true"` into the latter, throwing `UnsatisfiedDependencyException`. Fixed by setting `SPRING_APPLICATION_JSON='{"management":{"metrics":{"distribution":{"percentiles-histogram":{"http.server.requests":true,"jvm.gc.pause":true}}}}}'` on the `motd` Deployment instead (`gitops-repo/charts/motd/values.yaml`), which carries literal dotted/dashed keys and sidesteps the ambiguity. Verified live: `http_server_requests_milliseconds_bucket` now exposes real boundaries and the Duration-MAX panel returns real values instead of `NaN` | Yes |
| Logs / App | The `app` picker had exactly one (useless) option, and selecting it dumped every pod's logs from the whole cluster together — every pod got the same literal `job` label (the Alloy component's own name), because `loki.source.kubernetes "pods"` had no relabeling to turn Kubernetes discovery metadata into real labels. **Later (2026-07-13)**: checked this dashboard for the same class of issue as the metric-naming mismatches on the Prometheus-based dashboards, and found a LogQL equivalent - the Logs panel hardcoded a `\| logfmt` parser stage on every query, but this cluster's apps use inconsistent log formats (motd emits JSON, motd-ui emits nginx/Apache combined access-log format, only infra components are actually logfmt). Confirmed live that `\| logfmt` against a JSON-formatted line doesn't drop it, but silently mis-parses fragments of the JSON as fake key=value pairs rather than failing cleanly (`\| json` correctly parses motd's logs, but would be equally wrong for motd-ui's nginx-format ones - no single parser stage is right for every app here) | Added a `discovery.relabel "pods"` step in Alloy promoting `namespace`/`pod`/`container` into real labels and deriving `job = namespace/container` per pod. **Dashboard**: forked into `gitops-repo/observability/grafana/dashboards/logs-app.json` with the `\| logfmt` stage removed entirely - the raw log line always displays correctly regardless of format, so this is strictly safer than guessing a single parser for every app | Yes (was No) |
| K8S Dashboard for Alloy Metrics exported to Mimir (Microservices Overview) | Every panel hardcoded the original author's Mimir datasource UID via the newer multi-line `"datasource": {"type": ..., "uid": ...}` object form, which the chart's plain-string `datasource: Mimir` substitution can't rewrite (it needs the whole field on one line). Also filtered on a `cluster` label that `kube-state-metrics` doesn't set natively. Its Persistent Volume panels (`kubelet_volume_stats_*`) were also blank — that metric comes from kubelet's separate main `/metrics` endpoint, which Alloy wasn't scraping (only `/metrics/cadvisor`). **Later (2026-07-13)**: checked this dashboard for the same class of metric-naming mismatch found on the other two — found none (all 23 referenced `container_*`/`kube_*` metrics matched exactly what's scraped, and all 5 template variables consistently use `=~`, so it doesn't have the Kubernetes/Views/Global `=` vs `=~` bug either). Did find a different gap: the "Namespace Resource Statistics" panel's `kube_service_info`/`kube_configmap_info`/`kube_secret_info` series (3 of its 5) were blank because those three resource types weren't in kube-state-metrics' explicit `collectors:` allow-list | Forked into `gitops-repo/observability/grafana/dashboards/alloy-k8s-microservices.json` with every UID replaced by a `${MIMIR_UID}` token, resolved via the chart's slice-form `datasource:` substitution instead. Added `external_labels = { "cluster" = "dev" }` to Alloy's `prometheus.remote_write` block. Added a second node scrape (`discovery.relabel "kubelet"` + `prometheus.scrape "kubelet"`, same node-IP:10250 pattern as cadvisor, just `__metrics_path__ = "/metrics"`) for the volume-stats metrics. Added `configmaps`/`secrets`/`services` to `gitops-repo/observability/kube-state-metrics/values/dev.yaml`'s `collectors:` list | Yes |
...
### How motd's two env vars reach Spring Boot config

`gitops-repo/charts/motd/values.yaml`'s `env:` list sets `SPRING_PROFILES_ACTIVE` and `SPRING_APPLICATION_JSON`, which Spring Boot resolves through two different mechanisms:

- **`SPRING_PROFILES_ACTIVE`** is read like any other env var, via ordinary relaxed binding against `SystemEnvironmentPropertySource` — it maps straight onto the flat `spring.profiles.active` property (a `String`/`List<String>`, no nested map involved), so there's no ambiguity in how the env var name should be parsed.
- **`SPRING_APPLICATION_JSON`** is special-cased. `SpringApplicationJsonEnvironmentPostProcessor` fires very early (on `ApplicationEnvironmentPreparedEvent`, before the `ApplicationContext` exists), looks specifically for this variable (or the `spring.application.json` system property), parses its value as JSON, and recursively flattens the nested object into dotted property keys (e.g. `{"management":{"metrics":{"distribution":{"percentiles-histogram":{"http.server.requests":true}}}}}` → `management.metrics.distribution.percentiles-histogram.http.server.requests=true`). That flattened map becomes its own high-precedence `PropertySource`, ranking above `application.yml`/`application.properties`.

The reason `motd` needs the second mechanism at all: plain `SCREAMING_SNAKE_CASE` env vars can't reliably set `management.metrics.distribution.percentiles-histogram.<meter-id>` (see the JVM dashboard row above) — `percentiles-histogram` is a word-prefix of the sibling `percentiles` map (`Map<String, double[]>`), and Micrometer meter IDs like `http.server.requests` are themselves dotted, so `SystemEnvironmentPropertySource`'s underscore-to-dot/dash guessing has no reliable way to reconstruct the intended key boundaries. `SPRING_APPLICATION_JSON`'s keys come from literal JSON strings, so they preserve dots/dashes exactly as written and sidestep the ambiguity entirely. Worth reaching for any time an env-var-set property is a `Map` with dynamic/dotted keys — `management.metrics.*`, `logging.level.*`, and `spring.jpa.properties.*` are the other common examples of this class in a Spring Boot app.

### Scrape target portability

This project is meant to be reused as a template by other developers, not just run as this one fixed environment — so `gitops-repo/observability/alloy/values/dev.yaml`'s `node_exporter` and `kube_state_metrics` scrape blocks use Kubernetes endpoint discovery filtered on each chart's stable `app.kubernetes.io/name` label (`prometheus-node-exporter`, `kube-state-metrics`) and its Service's named port (`metrics`, `http` respectively), rather than a hardcoded `"<service>.<namespace>.svc.cluster.local:<port>"` address.

The original approach (a plain hardcoded address) is exactly what broke the Node Exporter Full dashboard once already (see the table above) — it assumed the Helm release name would always produce a specific chart fullname. The `app.kubernetes.io/name` label doesn't depend on the release/ArgoCD Application name at all, so this survives a template user renaming either Application. Confirm the real label/port name by rendering the chart (`helm template <chart> -f <values>`) before copying this pattern to a new exporter — don't assume it from memory.

This also fixed a latent correctness bug for node-exporter specifically: it runs as a DaemonSet (one pod per node), so scraping its Service's ClusterIP only ever reached whichever single pod kube-proxy routed that connection to — every `node_cpu_seconds_total` series carried one shared `instance` label instead of one per node, silently under-counting cluster CPU/memory. Endpoint discovery scrapes every node's pod individually. kube-state-metrics runs as a single-replica Deployment, so it didn't have this particular problem, but the naming fragility was identical.
...
| Node Exporter Full | Panels backed by `node_systemd_units`, `node_processes_*`, `node_intr_total`/`node_interrupts_total` (e.g. "Systemd Sockets Accepted/Refused", "IRQ Detail"), or similarly niche collectors | `prometheus-node-exporter`'s systemd, processes, and interrupts collectors are all disabled by default (not every node-exporter collector runs out of the box) — confirmed via `node_scrape_collector_success{collector="systemd"}` etc. not being registered at all (not failing - just never enabled). Enable the relevant `--collector.*` flag in `gitops-repo/observability/node-exporter/values/dev.yaml` if you actually need one of these |
| Node Exporter Full | The "Pressure"/"Pressure Stall Information" panels' IRQ line (`node_pressure_irq_stalled_seconds_total`) | Unlike the other four PSI series on this panel (cpu/memory/io waiting+stalled, all present), this one metric is absent - most likely a node_exporter-version/kernel-support gap specific to IRQ pressure tracking rather than a disabled collector (the `pressure` collector itself is clearly working, since the other four series are populated). The panel still renders correctly with 4 of 5 lines |
...
Kubernetes / Views / Global did hit a *different* class of bug that's easy to mistake for a naming mismatch at first glance (see the table above): a `multi: true` template variable interpolated with `=` instead of `=~`. That one isn't about metric names at all — it's about Grafana's variable-escaping behavior versus the operator a panel's query happens to use, and it's only exposed by *some* job-label values (ones containing regex metacharacters) and not others. Worth checking for on any dashboard using a `multi: true` variable filtered with plain equality, regardless of whether the metric names themselves match.
